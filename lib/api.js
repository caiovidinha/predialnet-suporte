...
/**
 * Encaminha um corpo multipart (FormData) sem definir Content-Type — o fetch
 * cuida do boundary. Usado no upload de anexos de chamados.
 */
export async function apiServerForm(method, path, formData) {
  return fetch(`${API_BASE}${path}`, {
    method,
    headers: { 'x-access-token': process.env.ADMIN_BYPASS_TOKEN },
    body: formData,
    cache: 'no-store',
  })
}
